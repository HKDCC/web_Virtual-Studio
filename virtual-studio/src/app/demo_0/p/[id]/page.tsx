...
export default async function Demo0NotionPageRoute(props: { params: Promise<{ id: string }>; searchParams: Promise<{ from?: string; embed?: string }> }) {
  ...
  const backUrl = from === "news" ? "/demo_0/aievolutionlog" : "/demo_0/archive";
    ...
    <>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 48px 80px", position: "relative" }}>
        {htmlContent ? (
          ...
          <>
            {headings.length > 0 && <TableOfContentsWrapper headings={headings} />}
            <NotionBlocks blocks={blocks} />
          </>
        )}
      </div>
