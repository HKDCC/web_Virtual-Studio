...
# 分别供 Wrangler/Cloudflare 与本地 Notion 脚本使用；填写 Token 和数据库 ID
...
| `npm run sync:notes` | 从本地源目录生成文章和兼容跳转页，不修改 Notion |
| `npm run sync:notes:notion` | 生成文章并显式同步 Notion；需要有效凭据 |
...
|-----------------|-------------|----------------|
| MagazineHeader | components/magazine/MagazineHeader.tsx | 导航头部 |
| MagazineFooter | components/magazine/MagazineFooter.tsx | 底部信息 |
