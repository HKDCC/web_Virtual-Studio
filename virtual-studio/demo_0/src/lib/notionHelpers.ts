...
export function getCoverUrl(page: { cover?: unknown; properties?: PageProps }): string | null {
  // Check native cover first
  if (isObj(page.cover)) {
    if (page.cover.type === "external" && isObj(page.cover.external)) {
      return (page.cover.external.url as string) || null;
    }
    if (page.cover.type === "file" && isObj(page.cover.file)) {
      return (page.cover.file.url as string) || null;
    }
  }
  
  // Fallback to custom 'Cover' property
  if (page.properties && page.properties["Cover"]) {
    const filesProp = page.properties["Cover"];
    if (isObj(filesProp) && Array.isArray(filesProp.files) && filesProp.files.length > 0) {
      const f = filesProp.files[0];
      if (isObj(f)) {
        if (f.type === "external" && isObj(f.external)) return (f.external.url as string) || null;
        if (f.type === "file" && isObj(f.file)) return (f.file.url as string) || null;
      }
    }
  }
  return null;
...
export function getIcon(page: { icon?: unknown }): string | null {
  if (isObj(page.icon)) {
    if (page.icon.type === "emoji") {
      return (page.icon.emoji as string) || null;
    }
    if (page.icon.type === "external" && isObj(page.icon.external)) {
      return (page.icon.external.url as string) || null;
    }
    if (page.icon.type === "file" && isObj(page.icon.file)) {
      return (page.icon.file.url as string) || null;
    }
  }
  return null;
}
